...
class FWUpdateWrapper:
    ...
    def firmware_update(self,
        ...
        firmware_body = FirmwareUpdateRequest(reboot_options=reboot_options,
                                              ...
                                              targets=[target_host])
        if job_name == "":
            job_name = 'Datacenter_Host_Host_id(' + str(host_id) + ")_" + str(datetime.datetime.now())
        if job_description == "":
...
if __name__ == "__main__":
    ...
    if ARGS.ip is not None and ARGS.vcusername is not None and ARGS.vcpassword is not None and ARGS.vcUUID is not None \
        ...
        # Getting the groupType
        response_content, status_code = FWUpdateWrapper(base_url=base_url, vcUsercredential=credential,
                                                        vCenterUUID=ARGS.vcUUID).get_group_detail(omevv_group_id)
        groupType = response_content["groupType"]
        # Checking host is from the datacenter group. Fail if the host from cluster is given
        if groupType != "DATACENTER_NONCLUSTER":
            print("ERROR: Provided host_id(" + str(ARGS.host_id) + ") does not belongs to a datacenter. Host group "
                                                                   "type is " + str(groupType))
            exit(1)
        ...
        if status_code == 200:
            if host_compliance["complianceStatus"] != "NonCompliant":
                print("Either the host compliance status is already compliant or Unknown or Not Applicable. Launch "
                      "firmware update wizard and verify update is available for any component in 'Select Firmware "
                      "Components' page. Firmware update cannot be attempted.")
                exit(1)
            else:
