...
class FWUpdateWrapper:
    ...
    def firmware_update(self,
        ...
        firmware_body = FirmwareUpdateRequest(reboot_options=reboot_options,
                                              ...
                                              targets=[target_host])
        if job_name == "":
            job_name = 'vSAN_Host_Host_id(' + str(host_id) + ")_" + str(datetime.datetime.now())
        if job_description == "":
...
if __name__ == "__main__":
    ...
    PARSER.add_argument("--vcusername", "-u", required=True, help="username of vcenter")
    PARSER.add_argument("--vcpassword", "-p", required=True, help="password of vcenter")
    PARSER.add_argument("--vcUUID", "-d", required=True, default=None, help="UUID of the relevant vCenter")
    PARSER.add_argument("--host_id", "-hid", type=int, required=True, default=None, help="hid of the host")
    ...
    PARSER.add_argument("--vsan_health", "-vsanhl", required=False,
                        default='True', help="Checks vSAN Object Data is healthy, if not healthy firmware update will "
                                             "be run(boolean). Default=True")
    PARSER.add_argument("--mm_count_check", "-mmcount", required=False,
                        default='True', help="Checks if there is any host within the cluster is in maintenance mode. "
                                             "If yes, firmware update will not be run(boolean). Default=True")
    PARSER.add_argument("--vsan_enter_mm", "-vsanmm", required=False, default='ENSURE_ACCESSIBILITY',
                        help="vSAN host enter maintenance mode option. Supported values are: ENSURE_ACCESSIBILITY, "
                             "FULL_DATA_MIGRATION, NO_DATA_MIGRATION. Default=ENSURE_ACCESSIBILITY")
    ...
    if ARGS.ip is not None and ARGS.vcusername is not None and ARGS.vcpassword is not None and ARGS.vcUUID is not None \
        ...
        # Checking provided host is vSAN host
        if not response_content[0]["vsanEnabled"]:
            print("ERROR: Provided host_id(" + str(ARGS.host_id) + ") does not belongs to vSAN cluster")
            exit(1)
        print("Host is a vSAN host")
        ...
        job_id, response_content, status_code = FWUpdateWrapper(base_url=base_url,
            ...
            enter_maintenance_mode_option=FirmwareUpdateRequestEnterMaintenanceModeOption(ARGS.vsan_enter_mm),
            maintenance_mode_count_check=ARGS.mm_count_check,
            checkv_san_health=ARGS.vsan_health
        )
