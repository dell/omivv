...
def get_vcenter_tree(ip_address, user_name, password, vcenter_ip,  vc_username, vc_domain, vc_password):
    ...
    try:
        ...
        if(registered_vcenter_list is None):
            ...
            # Gell all Drift Jobs
            all_drift_jobs = get_all_drift_jobs(ip_address, bearer_token)
            if all_drift_jobs is not None:
                all_d_jobs = json.loads(all_drift_jobs)
                for d_jobs in all_d_jobs:
                    d_job_id = d_jobs['id']
                    get_drift_job_deatil_by_id(ip_address, bearer_token, d_job_id)
...
if __name__ == '__main__':
    ...
    get_vcenter_tree(ARGS.ip, ARGS.user, ARGS.password,ARGS.vcenterip,  ARGS.vcusername, ARGS.vcdomain, ARGS.vcpassword)
