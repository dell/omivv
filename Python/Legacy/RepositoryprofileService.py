...
"""
SYNOPSIS:
   Script to create or modify the repository profile in OMIVV.

DESCRIPTION:
   This script exercises the Spectre REST API create or modify 
   the repository profile in OMIVV.. For authentication X-Auth is used over Basic
   Authentication
   Note that the credentials entered are not stored to disk.

EXAMPLE:
   python RepositoryprofileService.py  --ip <xx>  --user <username> --password <pwd> --vcenterip <vcenterIP>
...
def repositoryProfileService(ip_address, user_name, password, bearer_token, vcenter_ip,  vc_username, vc_domain, vc_password,
                   createrepoprofile, modifyrepoprofile):
    ...
    try:
        ...
        if(registered_vcenter_list is None):
            ...
            if(createrepoprofile != None):
                create_repository_profile(ip_address, local_bearer_token, createrepoprofile)
            """ Modify the created repository file """ 
            if(modifyrepoprofile != None):
                modify_repository_profile(ip_address, local_bearer_token, modifyrepoprofile)
...
def get_repository_profile(omivv_ip, bearer_token):
    """Get All Repository Profile"""
    url = "https://" + omivv_ip + "/Spectre/api/rest/v1/Services/PluginProfileService/RepositoryProfiles"
    ...
    if(response.status_code == 200):
        ...
        print("Get all repository profile failed.")
    
def create_repository_profile(omivv_ip, bearer_token, create_args_file):
    base_url = "https://" + omivv_ip + "/Spectre/api/rest/v1/Services/PluginProfileService/RepositoryProfiles"
...
def modify_repository_profile(omivv_ip, bearer_token, modify_args_file):
    """Modify Repository Profile"""
    """Open the JSON input file"""
    input_file = open(modify_args_file,"r") 
    input_data = json.load(input_file)
    base_url = "https://" + omivv_ip + "/Spectre/api/rest/v1/Services/PluginProfileService/RepositoryProfiles/" + input_data["id"] 
    modifybody = input_data["put_body"]
    head = {'Authorization': 'Bearer ' + bearer_token}
    jsonReponse = requests.put(base_url, json=modifybody, verify=False, headers=head)
    if (jsonReponse.status_code == 200):
        json_response = json.dumps(jsonReponse.json(), indent=4, sort_keys=True)
        print(json_response)
        print("Successfully modified the repository profile.")
        return json_response
    else:
        print("Error in modify the repository profile.")
        print(jsonReponse)
...
if __name__ == '__main__':
    ...
    PARSER.add_argument("--createrepoprofile", "-crp", required=False, default=None,
                        help="create repository profile")
    PARSER.add_argument("--modifyrepoprofile", "-mrp", required=False, default=None,
                        help="modify repository profile")
    ...
    repositoryProfileService(ARGS.ip, ARGS.user, ARGS.password, ARGS.bearertoken, ARGS.vcenterip,  ARGS.vcusername, ARGS.vcdomain, ARGS.vcpassword,
                     ARGS.createrepoprofile, ARGS.modifyrepoprofile)
