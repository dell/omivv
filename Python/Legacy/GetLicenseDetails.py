...
"""
SYNOPSIS:
   Script to get all the license uploaded in the OMIVV.

DESCRIPTION:
   This script exercises the Spectre REST API to get all the license 
   uploaded in the OMIVV.. For authentication X-Auth is used over Basic
   Authentication
   Note that the credentials entered are not stored to disk.

EXAMPLE:
   python GetLicenseDetails.py  --ip <xx>  --user <username> --password <pwd> --vcenterip <vcenterIP>
...
def get_vcenter_tree(ip_address, user_name, password, bearer_token, vcenter_ip,  vc_username, vc_domain, vc_password,licesne_id):
    ...
    try:
        ...
        if(registered_vcenter_list is None):
            ...
            # Get all Drift Jobs
            all_license = get_all_licenses(ip_address, local_bearer_token)
            if all_license is not None:
                if(licesne_id is None):
                    print(all_license)
                else:
                    all_d_jobs = json.loads(all_license)
                    for d_jobs in all_d_jobs:
                        if(licesne_id == d_jobs['id']):
                            get_license_deatil_by_id(ip_address, local_bearer_token, licesne_id)
...
def get_license_deatil_by_id(omivv_ip, bearer_token, licesne_id):
    ...
    url = "https://" + omivv_ip + "/Spectre/api/rest/v1/Services/LicenseService/Licenses/" + licesne_id;
    head = {'Authorization': 'Bearer ' + bearer_token}
    response = requests.get(url, verify=False, headers=head)
    if (response.status_code == 200):
        json_response = json.dumps(response.json(), indent=4, sort_keys=True)
...
if __name__ == '__main__':
    ...
    PARSER.add_argument("--licenseid", "-lid", required=False, default=None,
                        help="password of vcenter")
    
    ARGS = PARSER.parse_args()
    if(ARGS.password == None and ARGS.bearertoken == None):
        print("Pass either the password or the bearer token")
    get_vcenter_tree(ARGS.ip, ARGS.user, ARGS.password, ARGS.bearertoken, ARGS.vcenterip,  ARGS.vcusername, ARGS.vcdomain, ARGS.vcpassword, ARGS.licenseid)
