...
"""
SYNOPSIS:
   Script to get the cluster profile details by its ID.

DESCRIPTION:
   This script exercises the Spectre REST API to to get the cluster 
   profile details by its ID. For authentication X-Auth is used over Basic
   Authentication
   Note that the credentials entered are not stored to disk.

EXAMPLE:
   python GetClusterProfileDeatilsById.py --ip <xx> --user <username> --password <pwd> --vcenterip <vcenterIP>
...
def get_cluster_profile_details_by_id(ip_address, user_name, password, bearer_token, vcenter_ip,  vc_username, vc_domain, vc_password, cp_id):
    ...
    try:
        ...
        if(registered_vcenter_list is None):
            ...
            # Gell all Drift Jobs
            all_cp_jobs = get_all_cluster_profile_report(ip_address, local_bearer_token)
            if all_cp_jobs is not None:
                if(cp_id is None):
                    print(all_cp_jobs)
                    return
                else:
                    all_cp_jobsjson = json.loads(all_cp_jobs)
                    for c_jobs in all_cp_jobsjson:
                        if(cp_id == c_jobs['id']):
                            #print("The cluster profile with id " + cp_id + " is found")
                            get_cluster_profile_by_id(local_bearer_token, c_jobs['href'])
        """ Log out from OMIVV """
        if(bearer_token == None):
...
def get_all_cluster_profile_report(omivv_ip, bearer_token):
    """Get all cluster profile jobs"""
    url = "https://" + omivv_ip + "/Spectre/api/rest/v1/Services/PluginProfileService/ClusterProfiles"
    ...
    if (response.status_code == 200):
        ...
        print("Get all cluster profile call failed.")

def get_cluster_profile_by_id(bearer_token, cp_id_href):
    """ Get the drift job details"""
    head = {'Authorization': 'Bearer ' + bearer_token}
    response = requests.get(cp_id_href, verify=False, headers=head)
    if (response.status_code == 200):
        json_response = json.dumps(response.json(), indent=4, sort_keys=True)
        print(json_response)
        return json_response
    else:
        print("Get all cluster profile jobs with id failed.")
...
if __name__ == '__main__':
    ...
    PARSER.add_argument("--clusterprofileid", "-cId", required=False, default=None,
                        help="cluster profile job id")
    ARGS = PARSER.parse_args()
    if(ARGS.password == None and ARGS.bearertoken == None):
        print("Pass either the password or the bearer token")
    
    get_cluster_profile_details_by_id(ARGS.ip, ARGS.user, ARGS.password, ARGS.bearertoken, ARGS.vcenterip, ARGS.vcusername, ARGS.vcdomain, ARGS.vcpassword, ARGS.clusterprofileid)
